...
export const createGameBaseSchema = z.object({
  ...
  scheduledAt: z.date().refine((value) => value.getTime() > Date.now() + 60000, {
    message: "Scheduled at must be at least one minute in the future",
  }),
...
export function validateAttendanceRestriction(
  data: { requiresAttendanceScore: boolean; minimumAttendanceScore: number | null },
  ctx: z.RefinementCtx,
) {
...
export const createGameSchema = createGameBaseSchema.superRefine(validateAttendanceRestriction);
